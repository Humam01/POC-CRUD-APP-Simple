...
func main() { //func adalah method
	if err := godotenv.Load(".env"); err != nil { //mencari dan membaca file .env
	...
	config.DB.AutoMigrate(&models.Task{}) //AutoMigrate() otomatis membuat atau memperbarui tabel dalam database berdasarkan struktur  &models.Task{} membuat atau memperbarui tabel yang sesuai dengan struktur model Task yang telah didefinisikan di dalam package models.
	...
	//Start server
	port := os.Getenv("PORT")
	if port == "" { //callback jika tidak ditemukan variabel PORT di file .env, maka akan menggunakan port default 8080
		port = "8080"
	}
